...
description: >-
  ...
  onCardBrandChanged, onPostalCodeChange, defaultValues, focusField, Enter key / host form submit /
  ...
  verification, WalletCustomerCreateAttributes / WalletPostalAddress /
  phoneRequired / shippingAddressRequired, or debugging blank iframes /
...
Current packages: `@amos.com/amos-js` 0.11.27, `@amos.com/react-amos-js` 0.11.25, and `@amos.com/node` 0.1.69 (peer `>=0.1.65`). `@amos.com/node` is a **peer dependency** of both client SDKs (install it for OpenAPI types even in browser-only TypeScript). Prefer the installed package README + types over inventing APIs.
...
**Rules.** Stripe-style class names (`.Input`, `.Label`, `.Error`, …) mapped onto iframe slots — you cannot style the iframe DOM from the host. They override `themeVariables` for the properties they set. Values may be `var(--primary)` (allowlisted theme tokens, no fallback). Unknown selectors/properties are ignored. `--input-height` / `--floating-input-height` are a **minimum**; `.Input` `padding` / `fontSize` / `lineHeight` can grow the field. Floated input text padding is `--floating-value-padding-top` and `--floating-value-padding-bottom`. Selectors and declaration keys: [api-reference.md](api-reference.md).
...
**Loading skeletons (do not invent your own):** card/bank mounts show a field-shaped skeleton (sized from `appearance`, `additionalFields`, `billingAddressRequirement`). Google Pay / Apple Pay mounts show a **button-shaped** skeleton at `height` (default `"48px"`). The iframe stays `opacity: 0` at its default pixel height until appearance is applied, or for 1.5s after mount if appearance never acks; the SDK then reveals it. Do not overlay a host-page placeholder or hide the mount until “ready.” When Plaid Embedded Institution Search is showing, the SDK covers that **350px** slot with the same pulse until Plaid’s `onLoad` (1.5s fallback).
...
3. Optional: `onValidityChange={({ isValid }) => …}` to enable/disable the submit button. Card only: `onCardBrandChanged={({ brand }) => …}`. Card and bank: `onPostalCodeChange={({ postalCode, country }) => …}` when a finished billing postal code is committed or cleared. Modal: `onEscapeKeyPressed` to close it.
...
- Required: `amount` (**string** major-currency decimal, e.g. `"50.00"` for $50.00), `merchantName`, **`onConfirm`**. The iframe converts that string to cents in `paymentIntentCreateAttributes.amount` (`CreatePaymentIntentInput`) — forward those attributes to `POST /payment_intents` as `{ payment_intent }` as-is.
...
- `customerCreateAttributes` is **`WalletCustomerCreateAttributes`** (re-exported from both client SDKs), **not** Amos `CreateCustomerInput`. Map it on the server. Nested `billingAddress` / `shippingAddress` use Amos billing names (`address_line1`, `state`, `postal_code`). Do not read top-level `line1` / `region` / `postalCode`. `phone` and `shippingAddress` are omitted unless you opted in.
- Name, email, and billing address are **always** collected. **`phoneRequired` / `shippingAddressRequired`** are top-level mount options (not `buttonProps`), default **`false`**. Pass `phoneRequired: true` to collect a phone (Apple Pay previously always did). Pass `shippingAddressRequired: true` to collect shipping.
...
| `onPostalCodeChange` on every keystroke or on `defaultValues` | Fires when a finished billing postal code is committed or cleared (`postalCode: null` if it becomes incomplete). Not sent for incomplete keystrokes or `defaultValues`. Card and bank only. Debounce shipping-rate requests on the host. |
...
| Wallet `line1` / `region` / `postalCode` | Removed. Read `billingAddress` only. Card/bank `defaultValues.billingAddress.line1` is a different contract. |
| Apple Pay no longer collecting phone | Pass top-level `phoneRequired: true`. Omitted defaults to `false`. Do not put it in `buttonProps`. |
...
5. Read installed package versions if APIs look unfamiliar. Current clients use `confirmPayment` / `confirmSetup`, `isConfirmTimeout`, wallet **`onConfirm`** + **`WalletCustomerCreateAttributes`**, host-form Enter submit, `onEscapeKeyPressed`, `appearance.fonts` / `rules` / `--font-family` (**replace** `themeVariables`), `defaultValues`, `focusField`, `onValidityChange`, card `onCardBrandChanged`, card/bank `onPostalCodeChange`, `resetForm`, bank `requireAchVerification` + `intent` (Plaid Embedded Institution Search with a 350px SDK skeleton), and wallet `height` / `buttonProps` / `iframeProps` / **`phoneRequired`** / **`shippingAddressRequired`**. Peer `@amos.com/node` `>=0.1.65` (current 0.1.69).
